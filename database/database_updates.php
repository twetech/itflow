...
if (LATEST_DATABASE_VERSION > CURRENT_DATABASE_VERSION) {
    ...
    if (CURRENT_DATABASE_VERSION == '0.2.6') {
        ...
        file_put_contents("/var/www/develop.twe.tech/includes/config.php", "\$repo_branch = 'master';" . PHP_EOL, FILE_APPEND);
    ...
    if (CURRENT_DATABASE_VERSION == '0.3.5') {
        ...
        // Update config.php var with new version var for use with docker
        file_put_contents("/var/www/develop.twe.tech/includes/config.php", "\n\$installation_id = '$installation_id';" . PHP_EOL, FILE_APPEND);
  ...
  // Insert queries here required to update to DB version 1.1.1
    ...
    if (CURRENT_DATABASE_VERSION == '1.1.4') {
        // Insert queries here required to update to DB version 2
        

        // Then, update the database to the next sequential version
        mysqli_query($mysqli, "UPDATE `settings` SET `config_current_database_version` = '2.0.0.0'");
    }
