                        ...
                        <div class="row">
                            <div class="col-6">
                                ...
                                <select class="form-control" name="user_pay_type" id="user_pay_type">
                                    ...
                                    <option value="hourly" <?= $user['user_pay_type'] == 'hourly' ? 'selected' : ''; ?>>Hourly (Per Hour, Paid Bi-Weekly)</option>
                                </select>
                            </div>
                            <div class="col-6">
                                <label for="user_pay_rate">Pay Rate <?= $user['user_pay_type'] == 'hourly' ? 'Per Hour' : 'Per Month'; ?></label>
                                <input class="form-control" name="user_pay_rate" id="user_pay_rate" value="<?= $user['user_pay_rate']; ?>">
                            </div>
                            <?php
                            // if user_pay_type is hourly, show max hours per week
                            if ($user['user_pay_type'] == 'hourly') { ?>
                                <div class="col-6">
                                    <label for="user_max_hours_per_week">Max Hours Per Week</label>
